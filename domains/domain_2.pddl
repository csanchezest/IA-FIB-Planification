(define (domain hotel)
  (:requirements 
      :adl 
      :typing
      :fluents
      :strips
  )
  (:types 
      habitacion reserva dias - object
  )
  (:functions 
      (capacityH ?h - habitacion)
      (capacityR ?r - reserva)
      (init-day ?r - reserva)
      (end-day ?r - reserva)
      (get-value ?d - dias)
      (get-orientacion ?h - habitacion)
      (get-preferencia ?r - reserva)
      (total-cost)
  )
  (:predicates
      (reservado ?r - reserva) ;; asignacion de reserva
      (incompatible ?r - reserva) ;; una reserva no se puede asignar a ninguna habitacion
      (ocupado ?h - habitacion ?d - dias) ;;una habitacion esta reservada durante un dia determinado
  )  
  (:action reservar-habitacion
    :parameters (?h - habitacion ?r - reserva)
    :precondition (and (not(reservado ?r)) 
                     (>= (capacityH ?h) (capacityR ?r))
                     (forall (?d - dias) (imply (and (>= (get-value ?d) (init-day ?r)) (<= (get-value ?d) (end-day ?r))) (not (ocupado ?h ?d))))
                  )
    :effect (and 
                (forall (?d - dias)
                    (when(and(>= (get-value ?d) (init-day ?r)) (<= (get-value ?d) (end-day ?r))) 
                        (ocupado ?h ?d))
                )
                (reservado ?r)
                (when (= (get-preferencia ?r) (get-orientacion ?h)) (increase (total-cost) 0.1))
                (when (not (= (get-preferencia ?r) (get-orientacion ?h))) (increase (total-cost) 0.5))
            )
   )
   (:action descarte-reserva
    :parameters (?r - reserva)
    :precondition (and 
                     (not(reservado ?r)) 
                     (forall (?h - habitacion) 
                        (or (< (capacityH ?h) (capacityR ?r))
                        (exists (?d - dias) (and (>= (get-value ?d) (init-day ?r)) (<= (get-value ?d) (end-day ?r)) (ocupado ?h ?d)))
                        )
                     )
                  )
    :effect (and
                (incompatible ?r)
                (increase (total-cost) 1)
            )
   )
)

